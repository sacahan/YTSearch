...
4. **當** 搜尋無結果 **則** 返回空清單 `{"videos": [], "message": "未找到符合條件的影片"}` 並附帶說明訊息
5. **當** 參數無效 **則** 返回清楚的錯誤訊息說明問題
6. **當** 搜尋返回超過 100 條結果 **則** 只返回前 100 條，並在訊息中標記「已達到上限，部分結果被截斷」
...
1. **當** 傳入空白關鍵字 **則** 返回錯誤訊息 `{"error": "INVALID_KEYWORD", "message": "搜尋關鍵字不能為空，請提供 1-200 字符的關鍵字"}` 說明關鍵字不能為空
2. **當** limit 參數超出允許範圍 **則** 返回錯誤訊息 `{"error": "INVALID_LIMIT", "message": "limit 必須在 1-100 之間，當前值：{value}"}` 說明有效範圍
3. **當** sort_by 參數不在允許的選項中 **則** 返回錯誤訊息 `{"error": "INVALID_SORT", "message": "sort_by 只能是 'relevance' 或 'date'，當前值：{value}"}` 列出有效選項
4. **當** YouTube 服務暫時無法使用 **則** 返回錯誤訊息 `{"error": "SERVICE_UNAVAILABLE", "message": "YouTube 服務暫時無法使用，已重試 3 次，請稍後再試"}` 說明服務狀態並建議稍後重試
5. **當** 快取服務故障 **則** 降級到直接搜尋，返回正常結果，記錄 WARN 級別日誌 "快取服務故障，使用直接搜尋"
6. **當** 收到畸形 JSON 請求 **則** 返回 MCP 錯誤 `{"error": "INVALID_REQUEST", "message": "無效的請求格式: 缺少必要參數或類型不匹配"}` ，而非伺服器崩潰
...
- 當 MCP 客戶端在搜尋過程中斷開連接時,系統偵測到連接中斷，立即停止進行中的操作，釋放資源，並記錄斷開事件
- 當同時有多個 MCP 客戶端連接時,系統使用非同步隊列管理，每個連接獨立處理，支援至少 10 個並發連接
- 當搜尋返回非常大的結果集時,MCP 工具限制單次返回的結果數量（最多 100 條），避免超出 MCP 訊息大小限制
- 當 YouTube 網站結構改變導致爬蟲失效時,系統返回明確的錯誤訊息「YouTube 服務暫時無法使用」，建議使用者稍後重試，並記錄為 ERROR 級別日誌
- 當 Redis 快取服務不可用時,MCP 工具自動降級到直接搜尋，記錄 WARN 級別日誌「快取服務故障，使用直接搜尋」，搜尋仍能正常完成
- 當關鍵字包含特殊字符或不同語言文字時,系統接受 UTF-8 編碼的任意字符（包括中文、日文、表情符號等），進行 URL 編碼後傳遞給爬蟲
...
- **FR-013**: 系統必須支援 HTTP 傳輸模式（StreamableHTTPSessionManager），MVP 階段不支援 stdio 或 SSE 模式
- **FR-014**: 系統必須在接收到畸形請求時返回有效的 MCP 錯誤回應，而非伺服器崩潰或無回應
- **FR-015**: 系統必須偵測連接中斷，在 1 秒內釋放相關資源，防止孤立連接洩漏
...
- **SC-009**: 客戶端連接中斷時，MCP 伺服器在 1 秒內偵測並釋放資源，不會留下孤立連接
- **SC-010**: 畸形請求的拒絕率達到 100%，所有無效請求都返回有意義的錯誤訊息而非伺服器崩潰
...
- **Q5: MCP 傳輸模式支援** → **A: 僅支援 HTTP（StreamableHTTPSessionManager）**
  - 決策：MVP 階段只支援 HTTP 傳輸模式，不支援 stdio 或 SSE
  - 理由：
    - HTTP 模式與現有 FastAPI 應用無縫整合（技術決策已確定）
    - stdio 需額外的進程管理層，超出 MVP 複雜度
    - SSE 是單向推送，無法滿足 MCP 的雙向通信需求
  - 後續迭代：可在 P3 或後續版本中考慮 stdio 支援用於本地插件集成

- **Q6: MCP 服務類型與 REST API 的共存策略** → **A: HTTP MCP 服務 + 保留原有 REST API 服務**
  - **MCP 服務提供的類型**：
    - **HTTP 服務（StreamableHTTPSessionManager）**：提供給 MCP 客戶端（如 Claude Desktop）使用，支援 MCP 協定的工具呼叫和資源訪問
    - **不支援 SSE 和 stdio**：MVP 階段限制於 HTTP 模式，後續迭代可考慮擴展
  - **REST API 服務的持續可用性**：
    - 現有的 REST API 端點（`/api/v1/search` 等）**完全保留並繼續運作**，不受 MCP 功能影響
    - 使用者可以繼續透過 REST API 直接呼叫 YouTube 搜尋功能
    - MCP 伺服器和 REST API 伺服器獨立部署或整合於同一 FastAPI 應用中運作，互不干擾
  - **架構選項**：
    - **整合方案**（推薦）：在同一 FastAPI 應用中同時部署 REST API 路由和 MCP HTTP 路由（路徑如 `/mcp/*`），共享底層搜尋服務邏輯
    - **分離方案**：MCP 伺服器和 REST API 伺服器作為獨立程序運作，各自透過不同端口或主機提供服務
  - **相容性承諾**：新增 MCP 功能不涉及修改現有 REST API 的簽名、參數或行為，確保向後相容性
